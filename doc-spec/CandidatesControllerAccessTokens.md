...
Finds an existing candidate in CRM by email, generates a 6-digit TOTP PIN from their data + server `TOTP_SECRET_KEY`, emails it to them via GOV.UK Notify (fire-and-forget), and returns `204 No Content`. The PIN is later exchanged at `exchange_access_token` endpoints to prove email ownership and get pre-populated data.
